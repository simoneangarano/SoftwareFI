...
class ConvInjector(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, error_model='random', inject_p=0.01, inject_epoch=0, bias=False, **kwargs):
        super(ConvInjector, self).__init__()
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride, padding, bias=bias, **kwargs)
...
class LinearInjector(nn.Module):
    def __init__(self, in_channels, out_channels, error_model, inject_p=0.01, inject_epoch=0, **kwargs):
...
class SequentialInjector(nn.Module):
    ...
    def forward(self, x, inject=True, current_epoch=0, counter=0, inject_index=0):
        for layer in self.layers:
            x, counter, inject_index = layer(x, inject, current_epoch, counter, inject_index)
        return x, counter, inject_index
...
class SE(nn.Module):
    ...
    def __init__(self, num_out, ratio=4, error_model='random', inject_p=0.01, inject_epoch=0):
        super(SE, self).__init__()
        num_mid = _make_divisible(num_out // ratio)
        self.pool = GlobalAvgPooling()
        self.conv_reduce = ConvInjector(in_channels=num_out, out_channels=num_mid,
                                     kernel_size=1, bias=True, padding_mode='zeros',
                                     error_model=error_model, inject_p=inject_p, inject_epoch=inject_epoch)
        self.act1 = Activation('relu')
        self.conv_expand = ConvInjector(in_channels=num_mid, out_channels=num_out,
                                     kernel_size=1, bias=True, padding_mode='zeros',
                                     error_model=error_model, inject_p=inject_p, inject_epoch=inject_epoch)
    ...
    def forward(self, x, inject, current_epoch, counter, inject_index):
        ...
        out = self.pool(x)
        out, counter, inject_index = self.conv_reduce(out, inject, current_epoch, counter, inject_index)
        out = self.act1(out)
        out, counter, inject_index = self.conv_expand(out, inject, current_epoch, counter, inject_index)
        out = self.act2(out)
        out = x * out
...
class ConvUnit(nn.Module):
    ...
    def __init__(self, num_in, num_out, kernel_size=1, stride=1, padding=0, num_groups=1,
                 use_act=True, act_type='relu', error_model='random', inject_p=0.01, inject_epoch=0):
        super(ConvUnit, self).__init__()
        self.conv = ConvInjector(in_channels=num_in,
...
class GhostModule(nn.Module):
    ...
    def forward(self, x, inject, current_epoch, counter, inject_index):
        ...
        x1, counter, inject_index = self.primary_conv(x, inject, current_epoch, counter, inject_index)
        x2, counter, inject_index = self.cheap_operation(x1, inject, current_epoch, counter, inject_index)
...
class GhostModuleMul(nn.Module):
    ...
    def forward(self, x, inject, current_epoch, counter, inject_index):
        ...
        res, counter, inject_index = self.short_conv(res, inject, current_epoch, counter, inject_index)
...
class GhostBottleneck(nn.Module):
    ...
    def __init__(self, num_in, num_mid, num_out, kernel_size, stride=1, act_type='relu', use_se=False, layer_id=None,
        ...
        self.use_se = use_se
        if use_se:
            self.se = SE(num_mid, error_model=error_model, inject_p=inject_p, inject_epoch=inject_epoch)
    ...
    def forward(self, x, inject, current_epoch, counter, inject_index):
        """ forward of ghostnet """
        shortcut = x
        out, counter, inject_index = self.ghost1(x, inject, current_epoch, counter, inject_index)
        if self.use_dw:
            out, counter, inject_index = self.dw(out, inject, current_epoch, counter, inject_index)
        if self.use_se:
            out, counter, inject_index = self.se(out, inject, current_epoch, counter, inject_index)
        out, counter, inject_index = self.ghost2(out, inject, current_epoch, counter, inject_index)
        if self.down_sample:
            shortcut, counter, inject_index = self.shortcut(shortcut, inject, current_epoch, counter, inject_index)
        out = shortcut + out
        return out, counter, inject_index
...
class GhostNet(nn.Module):
    ...
    def __init__(self, model_cfgs, num_classes=1000, multiplier=1., final_drop=0., error_model='random', inject_p=0.1, inject_epoch=0):
        ...
        self.conv_stem = ConvInjector(in_channels=first_conv_in_channel,
        ...
        self.conv_head = ConvInjector(in_channels=output_channel,
    ...
    def forward(self, x, inject=True, current_epoch=0, counter=0, inject_index=0):
        """ forward of GhostNet """
        x, counter, inject_index = self.conv_stem(x, inject, current_epoch, counter, inject_index)
        x = self.bn1(x)
        x = self.act1(x)
        x, counter, inject_index = self.blocks(x, inject, current_epoch, counter, inject_index)
        x = self.global_pool(x)
        x, counter, inject_index = self.conv_head(x, inject, current_epoch, counter, inject_index)
        x = self.act2(x)
        x = self.squeeze(x)
        if self.final_drop > 0:
            x = self.dropout(x)
    ...
    def _make_layer(self, kernel_size, exp_ch, out_channel, use_se, act_func, stride=1, layer_id=0,
                    error_model='random', inject_p=0.1, inject_epoch=0):
        mid_planes = exp_ch
    ...
    def _initialize_weights(self):
        ...
        for m in self.modules():
            if isinstance(m, ConvInjector):
                ...
                nn.init.normal_(m.conv.weight, std=np.sqrt(2. / n))
